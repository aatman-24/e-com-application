...
DEFAULT_SKU_ORDER_PATH = os.path.join(PROJECT_DIR, "data/sku_order.txt")
...
class ReportGenerator(QtCore.QObject):
    ...
    def __init__(self, orders_paths, sku_map_path, output_path, sku_order_path=None):
        super().__init__()
        self.orders_paths = orders_paths
        self.sku_map_path = sku_map_path
        self.output_path = output_path
        self.sku_order_path = sku_order_path
    ...
    def run(self):
        try:
            ...
            # --- Sorting Logic ---
            sku_order = []
            if self.sku_order_path and os.path.exists(self.sku_order_path):
                self.progress.emit(f"Applying custom SKU order from: {self.sku_order_path}")
                try:
                    with open(self.sku_order_path, 'r') as f:
                        sku_order = [line.strip() for line in f if line.strip()]
                except Exception as e:
                    self.progress.emit(f"⚠️ Failed to read SKU order file: {e}")

            # Create a sorting key function
            sku_to_rank = {sku: i for i, sku in enumerate(sku_order)}
            
            def sku_sort_key(item):
                sku = item[0]
                return sku_to_rank.get(sku, 999999), sku

            sorted_report_items = sorted(report_data.items(), key=sku_sort_key)
            # ---------------------

            for sku, size_dict in sorted_report_items:
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.sku_order_edit = QtWidgets.QLineEdit(DEFAULT_SKU_ORDER_PATH)
        self.sku_order_btn = QtWidgets.QPushButton('Browse')
        self.sku_order_btn.clicked.connect(self.browse_sku_order)
        h3 = QtWidgets.QHBoxLayout()
        h3.addWidget(self.sku_order_edit)
        h3.addWidget(self.sku_order_btn)
        form.addRow('SKU Order TXT:', h3)
    ...
    def browse_sku_order(self):
        path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self, 'Select SKU Order TXT', '', 'Text Files (*.txt);;All Files (*)'
        )
        if path:
            self.sku_order_edit.setText(path)
    ...
    def generate_report(self):
        ...
        self.worker = ReportGenerator(
            orders_paths, 
            self.sku_edit.text(), 
            output_path, 
            sku_order_path=self.sku_order_edit.text().strip()
        )
