#!/usr/bin/env python3
"""
Order Report Generator - PyQt5 Desktop UI

Features:
- Select Orders CSV
- Select SKU mapping CSV (parent_sku,child_sku)
- Select output report path (text file)
- Generate report using filtering and size-mapping logic you specified
- Logs area showing processing details
- Buttons to open generated report or open output folder

Dependencies: pandas, PyQt5
Install: pip3 install pandas PyQt5

Save this file and run: python3 generate_order_report_ui.py
"""
...
# Default file paths (can be changed in UI)
DEFAULT_ORDERS_PATH = os.path.expanduser("~/Documents/orders.csv")
DEFAULT_SKU_MAP_PATH = os.path.expanduser("~/Documents/sku_mapping.csv")
DEFAULT_OUTPUT_PATH = os.path.expanduser("~/Documents/order_report.txt")
...
def load_sku_mapping(csv_path: str) -> dict:
    """Load parent->children mapping from CSV."""
...
def normalize_size(size: str) -> str:
    """Robust normalization handling Years/Months/various formats."""
    if pd.isna(size):
        return str(size)
    ...
    # common variants with dash or slash
    s = s.replace('/', '-')

    # handle month-edge cases explicitly
...
def include_row(row) -> bool:
    """Apply inclusion rules: PENDING/HOLD include; CANCELLED ignore;
       READY_TO_SHIP include only if packet id empty/null."""
...
# ------------------------------
# Worker: generate report (same logic as before)
# ------------------------------
class ReportGenerator(QtCore.QObject):
    ...
    def run(self):
        try:
            ...
            self.progress.emit(f"Loading orders from: {self.orders_path}")
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def browse_orders(self):
        path, _ = QtWidgets.QFileDialog.getOpenFileName(self, 'Select Orders CSV', os.path.expanduser('~'), 'CSV Files (*.csv);;All Files (*)')
        if path:
            self.orders_edit.setText(path)

    def browse_sku_map(self):
        path, _ = QtWidgets.QFileDialog.getOpenFileName(self, 'Select SKU Mapping CSV', os.path.expanduser('~'), 'CSV Files (*.csv);;All Files (*)')
        if path:
            self.sku_edit.setText(path)

    def browse_output(self):
        path, _ = QtWidgets.QFileDialog.getSaveFileName(self, 'Select Output Report File', self.output_edit.text(), 'Text Files (*.txt);;All Files (*)')
        if path:
    ...
    def generate_report(self):
        ...
        if not output_path:
            QtWidgets.QMessageBox.warning(self, 'Output required', 'Please select output report path.')
            return
    ...
    def on_finished(self, message):
        ...
        if message.startswith('SUCCESS') or message.startswith('SUCCESS') or message.startswith('SUCCESS'):
    ...
    def open_output_folder(self):
        path = self.output_edit.text().strip()
        if not path:
            QtWidgets.QMessageBox.warning(self, 'Missing', 'Please select an output file first.')
            return
...
def main():
    ...
    # Apply a modern-ish font and style tweaks for Option B
    app.setStyle('Fusion')
    palette = app.palette()
    # Slightly rounded and lighter UI by using Fusion with default palette
    window = MainWindow()
    window.show()
    sys.exit(app.exec_())
