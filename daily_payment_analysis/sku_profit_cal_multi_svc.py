...
SKU_PRICE_FILE = Path(os.environ.get("SKU_PRICE_FILE", "data/sku_price.csv"))


order_files_env = os.environ.get("ORDER_FILES")
if order_files_env:
    ORDER_FILES = [Path(p) for p in order_files_env.split(";")]
else:
    ORDER_FILES = sorted(Path("data/orders").glob("*.xlsx"))
...
# OUTPUT FILE (FROM UI OR DEFAULT)
OUTPUT_FILE = Path(
    os.environ.get(
        "OUTPUT_FILE",
        "data/sku_profit_report_fallback.xlsx"
    )
).expanduser().resolve()
...
print(">>> Writing Excel to:", OUTPUT_FILE)
