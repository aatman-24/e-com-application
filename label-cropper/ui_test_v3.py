...
class Worker(QObject):
    ...
    def run(self):
        try:
            ...
            #     cropped_fk = str(out_dir / f"{date_str}_flipkart_cropped_{hour_str}.pdf")
            ...
            #         crop_flipkart_labels(merged_fk, cropped_fk)
            #         process_flipkart_single_pass_4x6(cropped_fk, final_pdf)
            #         try:
            #             os.remove(merged_fk)
            #         except Exception:
            #             pass
            #     else:
            #         crop_flipkart_labels(self.files[0], cropped_fk)
            #         process_flipkart_single_pass_4x6(cropped_fk, final_pdf)

            elif self.platform.startswith("flipkart"):
                final_pdf = str(out_dir / f"{date_str}_{self.platform}_{hour_str}.pdf")
                self.log.emit(f"Flipkart ({self.platform}): processing {len(self.files)} file(s)...")

                cropped_fk = str(out_dir / f"{date_str}_flipkart_cropped_{hour_str}.pdf")
                temp_files.append(cropped_fk)

                if len(self.files) > 1:
                    
                    merged_fk = str(out_dir / f"{date_str}_flipkart_merged_{hour_str}.pdf")
                    temp_files.append(merged_fk)
                    merge_input_pdfs(self.files, merged_fk)
                ...
                if self.platform == "flipkart_4x4":
                    process_flipkart_single_pass(cropped_fk, final_pdf)
                else:
                    process_flipkart_single_pass_4x6(cropped_fk, final_pdf)
            ...
            for f in temp_files:
                try:
                    if f and os.path.exists(f):
                        os.remove(f)
                        # self.log.emit(f"🧹 Deleted temp file: {os.path.basename(f)}")
                except Exception as e:
                    print(e)
                    # self.log.emit(f"⚠️ Could not delete temp file {f}: {e}")
...
class LabelApp(QWidget):
    ...
    def _build_ui(self):
        ...
        self.radio_flipkart_4x4 = QRadioButton("Flipkart (4×4)")
        self.radio_flipkart_4x6 = QRadioButton("Flipkart (4×6)")

        # default selection
        self.radio_meesho.setChecked(True)
        self.radio_flipkart_4x4.setChecked(False)
        self.radio_flipkart_4x6.setChecked(False)
        ...
        self.radio_meesho.toggled.connect(
            lambda checked: setattr(self, "platform", "meesho" if checked else self.platform)
        )

        self.radio_flipkart_4x4.toggled.connect(
            lambda checked: setattr(self, "platform", "flipkart_4x4" if checked else self.platform)
        )

        self.radio_flipkart_4x6.toggled.connect(
            lambda checked: setattr(self, "platform", "flipkart_4x6" if checked else self.platform)
        )

        mode_row.addWidget(self.radio_meesho)
        mode_row.addWidget(self.radio_flipkart_4x4)
        mode_row.addWidget(self.radio_flipkart_4x6)
        mode_row.addStretch()
    ...
    def apply_dark_theme(self):
        ...
        for rb in [
            self.radio_meesho,
            self.radio_flipkart_4x4,
            self.radio_flipkart_4x6,
        ]:
