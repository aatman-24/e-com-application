...
PyQt5 GUI wrapper for:
 - generate N images by replacing background of an input image with light/studio colors
...
Usage:
  pip install PyQt5 pillow requests rembg onnxruntime
...
# ---------------------------
# Image generation (background replacement)
# ---------------------------
def generate_hsl_colors(count):
    # produce light HSL-based colors (low saturation, high value) for studio look
    colors = []
    for i in range(count):
        h = i / float(max(1, count))
        # low saturation (pastel), high brightness
        r, g, b = colorsys.hsv_to_rgb(h, 0.28, 0.95)
        colors.append((int(r*255), int(g*255), int(b*255)))
    return colors

# def generate_random_colors(count):
#     # light/pastel range
#     return [(random.randint(185,255), random.randint(185,255), random.randint(185,255)) for _ in range(count)]

def generate_random_colors(count):
    """
    Return `count` light pastel colors with good hue variation.
    Uses HSV: low saturation, high brightness.
    """
    colors = []
    for _ in range(count):
        h = random.random()                      # any hue 0..1
        s = random.uniform(0.12, 0.30)           # low saturation (soft)
        v = random.uniform(0.88, 0.98)           # high brightness (light)
        r, g, b = colorsys.hsv_to_rgb(h, s, v)
        colors.append((int(r*255), int(g*255), int(b*255)))
    return colors


def generate_distinct_pastels(count):
    """
    Produce count pastel colors spaced around the hue wheel (distinct)
    with slight random jitter, low saturation and high brightness.
    """
    colors = []
    for i in range(count):
        base_h = i / float(max(1, count))
        h = (base_h + random.uniform(-0.04, 0.04)) % 1.0
        s = random.uniform(0.12, 0.28)
        v = random.uniform(0.88, 0.98)
        r, g, b = colorsys.hsv_to_rgb(h, s, v)
        colors.append((int(r*255), int(g*255), int(b*255)))
    return colors
...
def generate_distinct_pastels_v2(count, min_dist=40, max_tries=120):
    """
    Produce `count` visibly distinct pastel/studio colors.

    - Evenly space base hues around the wheel for maximal separation.
    - Add a reasonably large jitter to hue so results don't look mechanical.
    - Vary saturation and value (kept in pastel/light ranges).
    - Reject new colors that are too close (Euclidean RGB distance) to any already chosen.
    - min_dist controls how different colors must be (0..~441). Increase for more variety.
    """
    def rgb_tuple_from_hsv(h, s, v):
        r, g, b = colorsys.hsv_to_rgb(h, s, v)
    ...
    for i in range(count):
        ...
        while not accepted and tries < max_tries:
            tries += 1
            # larger jitter so neighbors differ well
            h = (base_h + random.uniform(-0.18, 0.18)) % 1.0
            # pastel-ish but allow more visible saturation variation
            s = random.uniform(0.18, 0.45)   # 0.18..0.45
            v = random.uniform(0.88, 0.99)   # bright (studio)
            cand = rgb_tuple_from_hsv(h, s, v)

            # ensure candidate is not too close to any existing color
            too_close = False
            for ex in colors:
                if rgb_dist(cand, ex) < min_dist:
                    too_close = True
                    break
            if not too_close:
                colors.append(cand)
        ...
        # fallback: if we couldn't find a distant color after many tries, accept last candidate
        if not accepted:
            colors.append(cand)
...
# keep add_border around (unused) so other code remains familiar
...
class Worker(QtCore.QRunnable):
    ...
    def _run_pipeline(self) -> str:
        ...
        # Load source image once (keep as RGBA for alpha operations)
        src_path = Path(conf["input_image"])
        if not src_path.exists():
            raise FileNotFoundError(src_path)
        # open as RGBA so rembg has max info
        src_img = Image.open(src_path).convert("RGBA")

        # remove background once (returns RGBA with transparent background)
        self.signals.log.emit("Removing background (this may take a second)...")
        fg = remove(src_img)  # PIL Image (RGBA)
        ...
        for idx, color in enumerate(colors, 1):
            if self._is_abort:
                ...
                return ""
            # Create new background (RGB) with light/studio color
            bg = Image.new("RGB", fg.size, color)
            # Paste foreground using its alpha channel
            if fg.mode in ("RGBA", "LA") or ("transparency" in fg.info):
                bg.paste(fg, mask=fg.split()[3])
            else:
                # fallback if alpha not present
                bg.paste(fg)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Meesho Bulk Background Replace + Shipping Checker")
        ...
        default_out = str(Path(downloads_dir, "generated_bg").resolve())
    ...
    def start(self):
        ...
        config = {
            ...
            "fee_retries": 6,
            "fee_delay": 1.0,
            "fetch_dup_base": {"is_old_image_match_enabled": False, "sscat_id": 13864},
            "fee_base": {"sscat_id": 13864, "gst_percentage": 5, "price": 20, "supplier_id": 2989863, "gst_type": "GSTIN"},
        }
